...
from supervisely.project.download import download_to_cache, copy_from_cache, is_cached, get_cache_size


def _no_cache_download(
    api: sly.Api, project_info: sly.ProjectInfo, dataset_infos: List[sly.DatasetInfo], progress: Progress
):
...
def download_project(
    ...
    try:
        ...
        if len(cached) == 0:
            ...
            log_msg = "Using cached datasets: " + ", ".join(f"{ds_info.name} ({ds_info.id})" for ds_info in cached)
        sly.logger.info(log_msg)
        if len(to_download) == 0:
            log_msg = "All datasets are cached. No datasets to download"
        else:
            log_msg = "Downloading datasets: " + ", ".join(f"{ds_info.name} ({ds_info.id})" for ds_info in to_download)
        sly.logger.info(log_msg)
        # get images count
        ...
        with progress(
            message="Retreiving data from cache...", total=total, unit="B", unit_scale=True, unit_divisor=1024
        ) as pbar:
            dataset_names = [ds_info.name for ds_info in dataset_infos]
            copy_from_cache(
                project_id=project_info.id, dest_dir=g.project_dir, dataset_names=dataset_names, progress_cb=pbar.update
            )
    except Exception:
