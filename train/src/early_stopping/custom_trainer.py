...
from src.early_stopping.custom_torch_utils import EarlyStopping as CustomEarlyStopping
...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        self.stopper, self.stop = (
            CustomEarlyStopping(
                patience=self.args.patience, stop_event=self.stop_event
            ),
            False,
        )
            ...
            if f.exists():
                ...
                if f is self.best:
                    if self.last.is_file():  # update best.pt train_metrics from last.pt
                        k = "train_results"
                        torch.save(
                            {**torch.load(self.best), **{k: torch.load(self.last)[k]}},
                            self.best,
                        )
                    LOGGER.info(f"\nValidating {f}...")
