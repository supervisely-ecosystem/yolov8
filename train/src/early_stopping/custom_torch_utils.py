...
def get_cpu_info():
    ...
    with contextlib.suppress(Exception):
        ...
        k = (
            ...
            "hardware_raw",
            "arch_string_raw",
        )  # keys sorted by preference (not all keys always available)
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "") -> None:
    ...
    # Save
    torch.save(
        {**updates, **x}, s or f, use_dill=False
    )  # combine dicts (prefer to the right)
...
class EarlyStopping:
    ...
    def __init__(self, patience=50, stop_event=None):
        """
