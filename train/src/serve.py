...
class YOLOv8Model(sly.nn.inference.ObjectDetection):
    ...
    def _create_label(self, dto: Union[PredictionMask, PredictionBBox, PredictionKeypoints]):
        if self.task_type == "object detection":
            ...
            if obj_class is None:
                raise KeyError(f"Class {dto.class_name} not found in model classes {self.get_classes()}")
    ...
    def _to_dto(self, prediction, settings: dict) -> List[Union[PredictionMask, PredictionBBox, PredictionKeypoints]]:
            ...
            if len(self.class_names) == 1:
                ...
                for box, keypoints in zip(boxes_data, keypoints_data):
                    ...
                    if keypoints_data.shape[-1] == 3:
                        ...
                        for j, (point_coordinate, point_score) in enumerate(zip(point_coordinates, point_scores)):
                ...
                for box, keypoints in zip(boxes_data, keypoints_data):
                    ...
                    point_labels = [value["label"] for value in keypoints_template["nodes"].values()]
